...
def countComics(data):
    comicsCount = data['data']['results'][0]['comics']['available']
    return comicsCount

def getThumbnail(data):
    url_base = data['data']['results'][0]['thumbnail']['path']
    image_size = "/portrait_xlarge.jpg"
    wget.download(url_base+image_size, out=f"./thumbnails/{args.hero}.jpg")
...
def main():           
    ...
    ## display results
    #print(uncannyxmen)
    print(f"{args.hero.title()} has {countComics(uncannyxmen)} comics")
    print("Check your thumbnails directory for a new image")
    getThumbnail(uncannyxmen)
